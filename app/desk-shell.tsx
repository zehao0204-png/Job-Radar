        ...
        <div className="mx-auto grid max-w-[1480px] gap-7 px-5 py-7 lg:grid-cols-[218px_minmax(0,1fr)] lg:px-8 lg:py-9">
          ...
          <div className="min-w-0">
            ...
            <section className="mt-7 grid border-y border-[#cbd4e7] bg-white/55 sm:grid-cols-3">
              {[[String(openCount).padStart(2, '0'), '已核验开放', '来自官方公开信息'], [String(pendingCount).padStart(2, '0'), '等待核验', '不等于尚未开放'], [String(companies.length), '公司入口', '覆盖五个重点行业']].map(([value, label, note], index) => <div key={label} className={`grid grid-cols-[auto_1fr] items-center gap-4 px-5 py-5 ${index < 2 ? 'border-b border-[#d8dfed] sm:border-b-0 sm:border-r' : ''}`}><strong className="utility text-3xl font-medium tracking-[-.08em] text-[#3657d6]">{value}</strong><div><p className="text-xs font-bold text-[#313c51]">{label}</p><p className="mt-1 text-[10px] text-[#8b95a8]">{note}</p></div></div>)}
            </section>
